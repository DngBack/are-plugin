...
def evaluate_with_pipelines(
    ...
    filter_tail_by_experts: bool = True,
    top_k: int = 2
) -> List[RejectionCase]:
    """
    ...
    Args:
        ...
        filter_tail_by_experts: Nếu True, filter tail cases theo expert selection
        top_k: Số experts được chọn trong gating (thường là 2)
    """
    print(f"\n   Evaluating mode={mode}, rejection_rate={rejection_rate} with notebook pipelines (sequential)...")
    if filter_tail_by_experts:
        print(f"   Filtering tail cases: only keep if LogitAdjust in top-{top_k} experts")
    ...
    for idx in tqdm(range(len(images)), desc=f"Pipeline eval ({mode})"):
        ...
        if case_type is not None:
            # Kiểm tra điều kiện filter cho tail classes
            group_idx = class_to_group_np[true_label]
            group_str = "tail" if group_idx == 1 else "head"
            
            # Nếu là tail class và filter được enable
            if filter_tail_by_experts and group_str == "tail":
                # Lấy gating weights từ our_result
                gating_weights = our_result['gating_weights']  # [3] numpy array
                
                # Lấy top-k expert indices
                top_k_indices = np.argsort(gating_weights)[-top_k:][::-1]  # Descending order
                
                # Kiểm tra xem có LogitAdjust trong top-k không
                has_logitadjust = LOGITADJUST_IDX in top_k_indices
                
                if not has_logitadjust:
                    # Bỏ qua: chỉ có CE và BalancedSoftmax (hoặc không có LogitAdjust)
                    filtered_count += 1
                    continue
    ...
    if filter_tail_by_experts and filtered_count > 0:
        print(f"   Filtered out {filtered_count} tail cases (no LogitAdjust in top-{top_k} experts)")
...
def main():
    ...
    dataset = torchvision.datasets.CIFAR100(root="./data", train=False, download=False)
    class_to_group_np = class_to_group.cpu().numpy()
    base_dir = Path("./infer_samples_LA")  # Đổi folder thành infer_samples_LA
    ...
    for mode in modes:
        ...
        # CHỈ LẤY TAIL CASES
        tail_cases_to_save = [c for c in cases_to_save if c.group == "tail"]
        
        if len(tail_cases_to_save) == 0:
            print(f"   No tail cases to save for mode={mode}, rr={RR_TARGET_SAVE:.1f}")
            continue
        
        saved_count = 0
        for case in tail_cases_to_save:
            class_idx = case.true_label
            group_name = "tail"  # Chỉ tail thôi
        ...
        print(f"   Saved {saved_count} tail images to {base_dir / mode / 'tail'}")
