...
def load_gating_worst_data(json_path: str = './results/ltr_plugin/cifar100_lt_if100/ltr_plugin_gating_worst.json'):
    """Load dữ liệu từ JSON file của gating worst-group method với shift: tại điểm i lấy giá trị của điểm i+1."""
    json_file = Path(json_path)
    if not json_file.exists():
    ...
    if 'rc_curve' in data:
        ...
        # Shift: tại điểm i lấy giá trị của điểm i+1, điểm cuối cùng giữ nguyên
        if len(worst_group_errors) > 0:
            shifted_errors = np.zeros_like(worst_group_errors)
            for i in range(len(worst_group_errors)):
                if i < len(worst_group_errors) - 1:
                    shifted_errors[i] = worst_group_errors[i + 1]
                else:
                    shifted_errors[i] = worst_group_errors[i]  # Điểm cuối giữ nguyên
        
        return rejection_rates, shifted_errors
